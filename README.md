...
一个轻量的 Chrome 扩展：在 YouTube 视频链接上右键，**后台**打开一个新的 Gemini 标签页，自动使用 **快速/Flash 模型** 并发送 `总结视频 <URL>`。
...
- 点击后会直接在后台静默打开新 Gemini 标签页 → 使用快速/Flash 模型 → 自动发送 `总结视频 <URL>`。
...
- 浏览器在后台静默打开新 Gemini 标签页，加载完成后会自动使用快速/Flash 模型并发送提示词。
- 切换到该标签页即可看到生成结果。
...
- `content.js` — 注入到 `gemini.google.com`：取出当前 Tab 待发送的 prompt → 使用快速/Flash 模型 → 在 Quill 编辑器（`.ql-editor`）写入 → 点击发送按钮 → 等待 user-query 气泡确认提交成功。
...
- `已是快速/Flash 模型，跳过切换。` / `已切换到快速/Flash 模型。` —— 模型选择 OK
...
- 真实调用 `handleMenuClick`，等新 tab 出现，content.js 使用快速/Flash 模型 + 写入 + 提交，断言对话历史里出现真实的 user-query 气泡（不是输入框里的文字），并且模型选择器最终文字是 `Fast` / `快速` / `Flash`。
...
[gemini-tab] log [YouTube → Gemini] 已是快速/Flash 模型，跳过切换。
[gemini-tab] log [YouTube → Gemini] ✓ 已提交。
当前模型选择器文本: Flash
端到端测试通过：prompt 已提交，模型为快速/Flash。
```
