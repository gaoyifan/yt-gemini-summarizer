...
- `background.js` — Service Worker：注册右键菜单、通过 `chrome.cookies` 检查 Google 登录 cookie、未登录时通过 `chrome.scripting` 在当前 YouTube 页面 `alert()`、已登录则打开后台 Tab 并把 prompt 暂存到 `chrome.storage.session`。
...
| `cookies` | 读取 Google 登录 cookie，避免靠打开 Gemini 登录页判断状态 |
| `host_permissions: gemini.google.com/*` | 允许 content script 在 Gemini 页运行 |
| `host_permissions: accounts.google.com/*` | 允许 `chrome.cookies` 检查 Google 登录 cookie |
...
- 用全新空 profile 真实验证「没有 Google 登录 cookie 时，不开 Gemini tab、返回 needsLogin」（**Phase 0**）。
- 在 SW 里把登录态打桩为 `false`，验证分支逻辑稳定（**Phase A**）。
...
第一次运行时会打开一个 Gemini 页面让你登录；测试会复用这一个页面轮询登录状态，不会持续打开/关闭 tab。登录态保存在 `/tmp/yt-gemini-test-profile`，后续运行直接免登录。
